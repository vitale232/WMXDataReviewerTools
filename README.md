...
| Check Title                 | DR Check Type                         | Where Clause                                                                                                                | Additional Parameters                                                                                                                                                       |
|-----------------------------|---------------------------------------|-----------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Invalid Geometry            | Invalid Geometry                      | \(FROM\_DATE IS NULL\) OR \(FROM\_DATE < '01/01/2007'\) OR \(FROM\_DATE > TO\_DATE\) OR \(FROM\_DATE > CURRENT\_TIMESTAMP\)  |
| Non\-Linear Segments        | Non\-Linear Segment                   | \(FROM\_DATE IS NULL\) OR \(FROM\_DATE < '01/01/2007'\) OR \(FROM\_DATE > TO\_DATE\) OR \(FROM\_DATE > CURRENT\_TIMESTAMP\) |
| Self\-Intersection Check    | Polyline or Path Closes on Self Check | \(FROM\_DATE IS NULL\) OR \(FROM\_DATE < '01/01/2007'\) OR \(FROM\_DATE > TO\_DATE\) OR \(FROM\_DATE > CURRENT\_TIMESTAMP\)  |
| Domain Check                | Domain Check                          | \(FROM\_DATE IS NULL\) OR \(FROM\_DATE < '01/01/2007'\) OR \(FROM\_DATE > TO\_DATE\) OR \(FROM\_DATE > CURRENT\_TIMESTAMP\) |
| Duplicate Vertices          | Duplicate Vertices                    | \(FROM\_DATE IS NULL\) OR \(FROM\_DATE < '01/01/2007'\) OR \(FROM\_DATE > TO\_DATE\) OR \(FROM\_DATE > CURRENT\_TIMESTAMP\)  | Tolerance: 0\.0011 meters                                                                                                                                                   |
| Calibration Point on Routes | Geometry on Geometry                  | \(FROM\_DATE IS NULL\) OR \(FROM\_DATE < '01/01/2007'\) OR \(FROM\_DATE > TO\_DATE\) OR \(FROM\_DATE > CURRENT\_TIMESTAMP\)  | FC1: Calibration Points <br> FC2: Milepoint <br> Tolerance: 0\.0011 meters <br> Compare Attributes: Milepoint\.ROUTE\_ID=Calibration\_Point\.ROUTE\_ID                      |
| Cutbacks                    | Cutbacks Check                        | \(FROM\_DATE IS NULL\) OR \(FROM\_DATE < '01/01/2007'\) OR \(FROM\_DATE > TO\_DATE\) OR \(FROM\_DATE > CURRENT\_TIMESTAMP\)  | Min angle in degrees: 30                                                                                                                                                    |
| Length Check                | Evaluate Polyline Length Check        | \(FROM\_DATE IS NULL\) OR \(FROM\_DATE < '01/01/2007'\) OR \(FROM\_DATE > TO\_DATE\) OR \(FROM\_DATE > CURRENT\_TIMESTAMP\)  | Type: Polyline <br> Length: <=0\.001 miles                                                                                                                                  |
| Dangles                     | Find Dangles                          | \(FROM\_DATE IS NULL\) OR \(FROM\_DATE < '01/01/2007'\) OR \(FROM\_DATE > TO\_DATE\) OR \(FROM\_DATE > CURRENT\_TIMESTAMP\) | Dangle Tolerance: 0\.009 miles                                                                                                                                              |
| Monotinicity Check          | Monotinicity Check                    | \(FROM\_DATE IS NULL\) OR \(FROM\_DATE < '01/01/2007'\) OR \(FROM\_DATE > TO\_DATE\) OR \(FROM\_DATE > CURRENT\_TIMESTAMP\) | Evalueate: M values <br> Search Goal: Non\-monotonic features/Decreasing Values                                                                                             |
| Orphan Check                | Orphan Check                          | TO\_DATE IS NULL                                                                                                            |
| Invalid FROM\_DATE          | Execute SQL                           | \(FROM\_DATE IS NULL\) OR \(FROM\_DATE < '01/01/2007'\) OR \(FROM\_DATE > TO\_DATE\) OR \(FROM\_DATE > CURRENT\_TIMESTAMP\) |
| Invalid TO\_DATE            | Execute SQL                           | \(TO\_DATE < '01/01/2007'\) OR \(TO\_DATE < FROM\_DATE\) OR \(TO\_DATE > CURRENT\_TIMESTAMP\)                               |
...
| Route Not on Centerline     | Geometry on Geometry                  | \(FROM\_DATE IS NULL\) OR \(FROM\_DATE < '01/01/2007'\) OR \(FROM\_DATE > TO\_DATE\) OR \(FROM\_DATE > CURRENT\_TIMESTAMP\) | where\_clause: FC1 blank <br> FC1: Centerlines <br> FC2: Milepoint <br> Spatial Relation Check Type: Within <br> Attributes: None                                           |

### Invalid Internal Event Checks
| Check Title                                    | DR Check Type                                                   | Where Clause                                                                         | Additional Parameters                  |
|------------------------------------------------|-----------------------------------------------------------------|--------------------------------------------------------------------------------------|----------------------------------------|
| Event Check \- FC                              | Invalid Event Check                                             | TO\_DATE IS NULL                                                                     | "where\_clause: On event and Milepoint |
| Search Goals: Find orphans/Find overlaps       |
| Measure tolerance: 0\.000000621369949494949"   |
| Event Check \- Municipality                    | TO\_DATE IS NULL                                                | "where\_clause: On event and Milepoint                                               |
| Search Goals: Find orphans/Find overlaps       |
| Measure tolerance: 0\.000000621369949494949"   |
| Event Check \- Maintenance Jurisdiction        | TO\_DATE IS NULL                                                | "where\_clause: On event and Milepoint                                               |
| Search Goals: Find orphans/Find overlaps       |
| Measure tolerance: 0\.000000621369949494949"   |
| Event Check \- Owning Jurisdiction             | TO\_DATE IS NULL                                                | "where\_clause: On event and Milepoint                                               |
| Search Goals: Find orphans/Find overlaps       |
| Measure tolerance: 0\.000000621369949494949"   |
| Event Check \- Number of Through Lanes Reverse | \(TO\_DATE IS NULL\) AND \(DIRECTION = '0' OR DIRECTION = '2'\) | "where\_clause: TO\_DATE IS NULL on event, where clause from this table on Milepoint |
| Search Goals: Find orphans/Find overlaps       |
| Measure tolerance: 0\.000000621369949494949"   |
| Event Check \- Number of Through Lanes Primary | \(TO\_DATE IS NULL\) AND \(DIRECTION = '0' OR DIRECTION = '1'\) | "where\_clause: TO\_DATE IS NULL on event, where clause from this table on Milepoint |
| Search Goals: Find orphans/Find overlaps       |
| Measure tolerance: 0\.000000621369949494949"   |
| Event Check \- Width of Through Lanes Reverse  | \(TO\_DATE IS NULL\) AND \(DIRECTION = '0' OR DIRECTION = '2'\) | "where\_clause: TO\_DATE IS NULL on event, where clause from this table on Milepoint |
| Search Goals: Find orphans/Find overlaps       |
| Measure tolerance: 0\.000000621369949494949"   |
| Event Check \- Width of Through Lanes Primary  | \(TO\_DATE IS NULL\) AND \(DIRECTION = '0' OR DIRECTION = '1'\) | "where\_clause: TO\_DATE IS NULL on event, where clause from this table on Milepoint |
| Search Goals: Find orphans/Find overlaps       |
| Measure tolerance: 0\.000000621369949494949"   |
