...
+ When this README.md document is updated, make sure to run the `convert.js` script in `./docs/markdown-to-html-github-style-master`. It's a slightly modified version of the [Markdown to GitHub Style Web](https://github.com/KrauseFx/markdown-to-html-github-style) repository. Change into that directory, and run `node convert.js "NYSDOT Validations Toolbox README"`. This command will generate a static HTML document of this readme file, so that users without access to github.com will be able to view the documentation.
